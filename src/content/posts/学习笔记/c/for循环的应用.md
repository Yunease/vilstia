...
description: "for循环的应用 概述 For循环（可以理解为计数循环）注意事项：（对于循环） 当（条件）时，重复做（某件事情）循环，每次做完一个循环之后，使得（设置变量）发生变化。 由于循环计算的时候整个变量的运算都在循环内部，所以对于变量的调整可以放在循环内去做——但是！定义参数类型的时候呢~不能去循环括..."
---
